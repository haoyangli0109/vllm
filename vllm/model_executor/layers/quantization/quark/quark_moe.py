...
from vllm.model_executor.layers.quantization.utils.mxfp4_utils import (
    _swizzle_mxfp4, OCP_MX_BLOCK_SIZE)
    ...
    def get_moe_method(
        ...
        if quant_config._is_fp8_w8a8(weight_config, input_config):
            ...
            if model_type == "gpt_oss":
                return QuarkW4A4MXFp4MoEMethod_OSS(weight_config, input_config,
                                                   module.moe_config)
            else:
...
class QuarkW4A4MXFp4MoEMethod_OSS(QuarkMoEMethod):
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int,
        ...
        if current_platform.is_rocm():
            intermediate_size_per_partition_after_pad = round_up(
                intermediate_size_per_partition, 128) # 2880 -> 2944
        else:
            intermediate_size_per_partition_after_pad = round_up(
    ...
    def process_weights_after_loading(self, layer):
        ...
        #     weight_scale=w13_scale, act_scale=layer.w13_input_scale.to(torch.float8_e4m3fn), flex_ctx=FlexCtx(rhs_data=w13_flex))
        # self.w2_precision_config = PrecisionConfig(
        #     weight_scale=w2_scale, act_scale=layer.w2_input_scale.to(torch.float8_e4m3fn), flex_ctx=FlexCtx(rhs_data=w2_flex))
        ...
        self.w13_precision_config = PrecisionConfig(
            weight_scale=w13_scale,actflex_ctx=FlexCtx(rhs_data=w13_flex, lhs_data=lhs_data13))
        self.w2_precision_config = PrecisionConfig(
            weight_scale=w2_scale, flex_ctx=FlexCtx(rhs_data=w2_flex, lhs_data=lhs_data2))
    ...
    def apply(
        ...
        if enable_eplb:
            raise NotImplementedError(
                "EPLB not supported for `QuarkW4A4MXFp4MoEMethod_OSS` yet.")
